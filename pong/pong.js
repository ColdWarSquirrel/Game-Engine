...
game.mainLoopFunctions.push(function () {
    ...
    if (mode.players == 1) {
        ...
        if (game.keysDown.KeyW == true) {
            if (player.location.y > 0) {
                ...
                Engine.gameScreen.mousePos.y = (player.location.y + (player.scale.height / 2));
            }
        }
        if (game.keysDown.KeyW == true) {
            if ((player.location.y + player.scale.height) < Engine.gameScreen.height) {
                player.location.y += (player.speed.y * delta);
                Engine.gameScreen.mousePos.y = (player.location.y + (player.scale.height / 2));
            }
        }
        else {
            if (!(Engine.gameScreen.mousePos.y + (player.scale.height / 2) > Engine.gameScreen.height || Engine.gameScreen.mousePos.y - (player.scale.height / 2) < 0)) {
                player.location.y = (Engine.gameScreen.mousePos.y - (player.scale.height / 2));
            }
            else {
                if (Engine.gameScreen.mousePos.y + (player.scale.height / 2) > Engine.gameScreen.height) {
                    player.location.y = Engine.gameScreen.height - player.scale.height;
                }
                else if (Engine.gameScreen.mousePos.y - (player.scale.height / 2) < 0) {
                    player.location.y = 0;
                }
            }
        }
    ...
    if (mode.players < 2) {
        ...
        if (ball.location.x > Engine.gameScreen.width / 3) {
            ...
            if (Math.floor(adjustedEnemyLocation.y) > (Math.floor(ball.location.y))) {
                ...
                if ((enemy.location.y + enemy.scale.height) < Engine.gameScreen.height) {
                    enemy.location.y += (enemy.speed.y * delta);
                }
            }
        }
    ...
    if (ball.location.x <= 0 + (ball.scale.radius) || ball.location.x >= Engine.gameScreen.width - (ball.scale.radius)) { // wins or loses
        ...
        enemy.location.y = (Engine.gameScreen.height / 2) - (enemy.scale.height / 2);
        if (mode.players < 1) {
            player.location.y = (Engine.gameScreen.height / 2) - (player.scale.height / 2);
        }
    }
    else {
        if (isColliding) {
